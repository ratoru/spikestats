...
export function createStatsMap(games: Game[], players: Player[]): AllStats {
  ...
  //     return map;
  // }, {})
  let statsObj: AllStats = {
    totalGames,
    totalPoints,
    servingTeamWins,
    bestPlayer: { uuid: "", name: "" },
    bestTeam: ["", ""],
    stats: {},
  };
  // Add games.
  games.forEach((game) => {
    addStatsOfGame(statsObj, game);
  });
 ...
 */
export function updateBestPerformers(
  statsObj: AllStats,
  players: Player[]
): void {
...
export function addStatsOfGame(statsObj: AllStats, game: Game): void {
  ...
  addStatsForId(
    statsObj,
    game.blue_team.sort().join(""),
    game.score[0],
    game.score[0] > game.score[1]
  );
  addStatsForId(
    statsObj,
    game.red_team.sort().join(""),
    game.score[1],
    game.score[0] < game.score[1]
  );
}
 ...
 */
function addStatsForId(
  statsObj: AllStats,
  id: string,
  points: number,
  won: boolean
): void {
  let playerStats: IndividualStats = {
    gamesPlayed: 0,
    pointsContributed: 0,
    wins: [],
  };
  if (!(id in statsObj.stats)) {
    ...
    playerStats.wins.push(
      playerStats.wins[playerStats.gamesPlayed - 1] + (won ? 1 : 0)
    );
  }
...
function addServingTeamWin(statsObj: AllStats, won: boolean): void {
  if (statsObj.servingTeamWins.length === 0) {
    ...
    statsObj.servingTeamWins.push(
      statsObj.servingTeamWins[statsObj.servingTeamWins.length - 1] +
        (won ? 1 : 0)
    );
  }
}
...
function getBestPlayer(statsObj: AllStats, players: Player[]): Player {
  ...
  for (let i = 0; i < players.length; i++) {
    ...
    if (name in statsObj.stats) {
      ...
      let winLowerBound = lowerBound(
        playerStats.wins[playerStats.gamesPlayed - 1],
...
function getBestTeam(statsObj: AllStats, players: Player[]): Team {
  ...
  for (let i = 0; i < players.length - 1; i++) {
    for (let j = i; j < players.length - 1; j++) {
      let teamName: string = [players[i].uuid, players[j + 1].uuid]
        .sort()
        .join("");
      if (teamName in statsObj.stats) {
        let teamStats = statsObj.stats[teamName];
        let winLowerBound = lowerBound(
          teamStats.wins[teamStats.gamesPlayed - 1],
